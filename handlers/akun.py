...
async def cmd_akun(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = []
    if not member:
        rows.append([InlineKeyboardButton("Langganan VIP", callback_data="show_vip_menu")])
    rows.append([InlineKeyboardButton("Salin Link Referral", switch_inline_query=ref_link)])
