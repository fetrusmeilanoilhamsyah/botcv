...
def _clear_buffers(user_id: int):
    ...
    async def _bg_clear():
        try:
            import shutil
            await asyncio.to_thread(shutil.rmtree, ttv_dir, ignore_errors=True)
        except Exception:
            pass
    ...
    try:
        ...
        if delivery_mode == "zip":
            ...
            box_text = (
                ...
                f"• Nama Kontak : {contact_name_val}\n"
                f"• Jumlah / File : {per_file_val}\n"
                f"• Nama File VCF : {file_name_val}\n"
                f"• Urutan Mulai : {f'{awalan_val} (AWAL)' if data.get('file_num_style') == 'front' else f'{awalan_val} (AKHIR)'}\n"
